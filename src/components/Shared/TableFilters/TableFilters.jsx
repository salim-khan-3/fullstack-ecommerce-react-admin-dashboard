  ...
  const filterGroups = [
    {
      id: "show",
      label: "SHOW BY",
      options: ["12 Row", "24 Row", "50 Row"],
      current: "12 Row",
    },
    {
      id: "brand",
      label: "BRAND BY",
      options: ["Ecstasy", "Richman", "Lubana"],
      current: "Ecstasy",
    },
  ];
  return (
    <div className="p-8 bg-white/80 backdrop-blur-xl rounded-t-[2rem] border-x border-t border-gray-100 shadow-[0_20px_50px_rgba(0,0,0,0.05)]">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-10">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-2 ml-4 font-medium italic">
            "Real-time analytics & product tracking"
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
        ...
        <div className="relative">
          ...
          >
            ...
            <ChevronDown
              size={16}
              className={`text-gray-400 group-hover:text-blue-500 transition-transform duration-300 ${activeFilter === "cat" ? "rotate-180" : ""}`}
            />
          </button>
        ...
        {filterGroups.map((group) => (
          <div key={group.id} className="relative">
            ...
            <button
              onClick={() =>
                setActiveFilter(activeFilter === group.id ? null : group.id)
              }
              className="w-full flex items-center justify-between bg-gray-50/50 border border-gray-100 rounded-2xl py-3.5 px-5 text-sm font-bold text-gray-700 hover:bg-white hover:border-blue-200 hover:shadow-[0_10px_20px_rgba(0,0,0,0.04)] transition-all group"
            >
              <span className="group-hover:text-blue-600 transition-colors">
              ...
              <ChevronDown
                size={16}
                className={`text-gray-400 group-hover:text-blue-500 transition-transform duration-300 ${activeFilter === group.id ? "rotate-180" : ""}`}
              />
            </button>
