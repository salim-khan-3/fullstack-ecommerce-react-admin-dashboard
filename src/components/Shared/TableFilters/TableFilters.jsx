...
// onCategoryChange প্রপটি যোগ করা হয়েছে যা প্যারেন্ট কম্পোনেন্ট থেকে আসবে
const TableFilters = ({ onCategoryChange }) => {
...
useEffect(() => {
  const fetchCategories = async () => {
    try {
      ...
      const data = res?.categoryList || res?.data || res;
      setCategories(data);
  ...
  return (
    <div className="p-8 bg-white/80 backdrop-blur-xl rounded-t-[2rem] border-x border-t border-gray-100 shadow-[0_20px_50px_rgba(0,0,0,0.05)]">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
        ...
        {/* --- ডাইনামিক ক্যাটাগরি ড্রপডাউন (CATEGORY BY) --- */}
        <div className="relative">
          ...
          >
            <span className="group-hover:text-blue-600 transition-colors">{selectedCategory}</span>
            <ChevronDown size={16} className={`text-gray-400 group-hover:text-blue-500 transition-transform duration-300 ${activeFilter === "cat" ? 'rotate-180' : ''}`} />
          </button>

          {activeFilter === "cat" && (
            <div className="absolute top-full left-0 right-0 mt-3 bg-white rounded-2xl shadow-[0_25px_50px_-12px_rgba(0,0,0,0.15)] border border-gray-50 p-2 z-50 max-h-60 overflow-y-auto custom-scrollbar">
              <button 
              ...
              >
                All Categories
                {selectedCategory === "All Categories" && <Check size={14} className="text-blue-600" />}
              </button>
              ...
              {categories.map((cat) => (
                ...
                >
                  {cat.name}
                  {selectedCategory === cat.name && <Check size={14} className="text-blue-600" />}
                </button>
              ))}
            </div>
          )}
        ...
        {/* অন্যান্য স্ট্যাটিক ফিল্টার (Show By, Brand By) */}
        {filterGroups.map((group) => (
          <div key={group.id} className="relative">
            ...
            {activeFilter === group.id && (
              <div className="absolute top-full left-0 right-0 mt-3 bg-white rounded-2xl shadow-[0_25px_50px_-12px_rgba(0,0,0,0.15)] border border-gray-100 p-2 z-50">
      ...
      <div className="mt-10 pt-8 border-t border-gray-50 flex flex-col sm:flex-row justify-between items-center gap-4">
        ...
        <div className="flex items-center gap-3">
          <button 
            onClick={() => handleCategorySelect("All Categories", "")}
            className="flex items-center gap-2 px-6 py-3 rounded-2xl text-sm font-bold text-gray-500 hover:bg-gray-100 transition-all"
          >
            <X size={16} />
            Clear All
          </button>
...
// import React, { useState, useEffect } from "react";
// import { MoreHorizontal, Search, ChevronDown, Filter, X, Check } from "lucide-react";
// import { getAllCategories } from "../../../api/categoryApi"; // API ইম্পোর্ট করুন
...
//             <div className="absolute top-full left-0 right-0 mt-3 bg-white rounded-2xl shadow-[0_25px_50px_-12px_rgba(0,0,0,0.15)] border border-gray-50 p-2 z-50 max-h-60 overflow-y-auto custom-scrollbar">
//               <button 
//                 onClick={() => { setSelectedCategory("All Categories"); setActiveFilter(null); }}
//                 className="w-full flex items-center justify-between px-4 py-3 hover:bg-blue-50 rounded-xl text-sm font-semibold text-gray-600 hover:text-blue-700 transition-all text-left"
//               >
//                 All Categories
//                 {selectedCategory === "All Categories" && <Check size={14} className="text-blue-600" />}
