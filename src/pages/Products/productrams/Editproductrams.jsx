...
const EditProductRams = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        const res = await axiosInstance.get(`/productRams/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await axiosInstance.put(`/productRams/${id}`, { name });
    ...
    <div className="p-4 font-sans">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm px-6 py-4 flex flex-col md:flex-row md:items-center justify-between gap-3 mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-gray-800">Edit RAM</h1>
            <p className="text-xs text-gray-400">Update product RAM option</p>
          </div>
        </div>
        <nav className="flex items-center gap-1.5 text-xs">
          ...
          <ChevronRight size={12} className="text-gray-300" />
          <div className="px-3 py-1.5 rounded-lg bg-violet-600 text-white font-semibold shadow-sm">Edit RAM</div>
        </nav>
      </div>
      ...
      <div className="max-w-lg">
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="bg-gradient-to-r from-violet-600 to-purple-600 px-6 py-4">
            <h2 className="text-white font-bold text-base">Update RAM</h2>
...
export default EditProductRams;