...
const ProductSizeList = () => {
  ...
  const fetchRams = async () => {
    try {
      const res = await axiosInstance.get("/productSize");
      setRams(res.data || []);
    } catch {
      toast.error("Failed to load Size list");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = (id) => {
    Swal.fire({
      title: "Are you sure?",
      text: "This Size option will be deleted!",
      icon: "warning",
      ...
      if (result.isConfirmed) {
        try {
          await axiosInstance.delete(`/productSize/${id}`);
    ...
    <div className="p-4 font-sans">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm px-6 py-4 flex flex-col md:flex-row md:items-center justify-between gap-3 mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-gray-800">Product Size List</h1>
            <p className="text-xs text-gray-400">{rams.length} Size options available</p>
          </div>
        </div>
        <Link to="/productsize/add">
          <button className="flex items-center gap-2 bg-gradient-to-r from-blue-600 to-cyan-600 text-white px-5 py-2.5 rounded-xl text-sm font-bold shadow-md hover:opacity-90 transition-all">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="p-4 border-b border-gray-50">
          <div className="relative max-w-xs">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2.5 rounded-xl border border-gray-200 bg-gray-50 text-sm outline-none focus:border-blue-400 focus:bg-white transition-all"
            />
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead>
              <tr className="bg-gradient-to-r from-blue-600 to-cyan-600 text-white text-[11px] font-bold uppercase tracking-wider">
                <th className="py-4 px-6">#</th>
                <th className="py-4 px-6">Size Name</th>
            ...
            <tbody className="divide-y divide-gray-50">
              {loading ? (
                ...
                <tr><td colSpan={3} className="py-16 text-center text-gray-400 text-sm">No Size options found.</td></tr>
              ) : (
                  ...
                  <tr key={ram._id} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="py-4 px-6 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <Link to={`/productsize/edit/${ram._id}`}>
